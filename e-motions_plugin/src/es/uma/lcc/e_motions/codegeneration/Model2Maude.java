...
public class Model2Maude {
	...
	public static String model2Maude(IFile initialModel, IFile metamodel)
			throws IOException, URISyntaxException, CoreException, MaudelingException, ATLCoreException {
		...
		IFile paramsFile = FileManager.getDefault()
				.createParamsModelFile(ModuleGenerators.createParametersModel(modelName));
		...
		_fm.setInitialModelMaude(new Path("platform:/resource/" + project + "/" + FileManager.TMP_FOLDER + "/"
				+ FileManager.INITIAL_MODEL_MAUDE_FILENAME));
	...
	public static String model2MaudePalladio(IFile initialModel, IFile metamodel)
			throws IOException, URISyntaxException, CoreException, MaudelingException, ATLCoreException {
		...
		/* Loading Params model */
		// -> We create the params model file
		String modelName = PalladioRunningInformation.INITIAL_MODEL_NAME;
		modelName = modelName.substring(0, modelName.indexOf('.')).toUpperCase();

		IFile paramsFile = info.createParamsModelFile(ModuleGenerators.createParametersModel(modelName));
		...
		/* Extract resulting model */
		String project = info.getBehaviorModel().getProject().getName();

		info.setInitialModelMaude(new Path(
				"platform:/resource/" + project + "/.tmp/" + PalladioRunningInformation.INITIAL_MODEL_MAUDE_FILENAME));
		extractor.extract(maudeModel, info.getInitialModelMaude().toOSString());
