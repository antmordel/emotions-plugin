...
public class ModuleGenerators {
	...
	public static String tickRuleFilePalladio(final String NAME_INITIAL_MODULE, String behModuleName,
			String initialModel, String initTerm) {
		StringBuilder generatedTickModule = new StringBuilder();
		generatedTickModule.append(Printer.header());
		PalladioRunningInformation info = PalladioRunningInformation.getDefault();

		if (info.isAppliedRules()) {
			generatedTickModule.append("set print attribute on .\n");
		} else {
			generatedTickModule.append("set print attribute off .\n");
		}

		if (info.isShowAdvisories()) {
			generatedTickModule.append("set show advisories on .\n");
		} else {
			generatedTickModule.append("set show advisories off .\n");
		}
		...
		generatedTickModule.append("load ").append(info.getMetamodel().getName()).append(".maude").append("\n");
		generatedTickModule.append("load PalladioGrammar.maude\n");
		generatedTickModule.append("load ").append(info.getBehaviorModel().getName()).append(".maude\n");

		generatedTickModule.append("load ").append("flattenPalladio.xmi.maude\n");
		...
		generatedTickModule.append("mod TICKRULE is\n").append("  pr ").append(NAME_INITIAL_MODULE).append(" .\n")
				...
				.append("  var MODEL : @Model .\n\n").append("  crl [tick] :\n ").append("   { MODEL } in time T\n")
				.append("  =>\n").append("    { delta(MODEL, TE) } in time (T plus TE)\n");
		if (info.getLimitTime() == -1) {
			generatedTickModule.append("  if TE := mte(MODEL) /\\ TE =/= zero /\\ (T plus TE) le ")
					.append(info.getLimitTime()).append(" .\n");
		} else {
			generatedTickModule.append("  if TE := mte(MODEL) /\\ TE =/= zero .\n");
		}
		generatedTickModule.append("endm\n\n");
