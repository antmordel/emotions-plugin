...
public class PalladioJob extends Job {
	...
	private void flattenPalladioFiles() {
		...
		outFlatten.setResource(rs.createResource(URI.createURI("file:///" + info.getOutputFolder() + "/outFlatten.xmi")));
		 ...
		 * 
		 */
		EcoreUtil.resolveAll(inSystem.getResource().getContents().get(0).eContents().get(0));
		console.println(inSystem.getResource().getContents().get(0).eContents().get(0).toString());
